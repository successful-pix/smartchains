...
function Auth() {
  ...
  function switchMode(next: Mode) { setMode(next); setShowPassword(false); }
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (mode === "login") {
        ...
        if (!data.user.email_confirmed_at) { switchMode("verify"); toast.info("Please verify your email before continuing."); return; }
        toast.success("Welcome back to SmartChain"); void navigate({ to: "/", replace: true });
      } else if (mode === "signup") {
        const selectedCountry = countries.find((c) => c.code === country);
        const { data, error } = await supabase.auth.signUp({ email: email.trim(), password, options: { emailRedirectTo: `${window.location.origin}/auth`, data: { display_name: fullName.trim(), phone: phone.trim(), country: selectedCountry?.name || country, country_code: countryCode, country_iso: country } } });
        if (error) throw error;
        if (!data.user) throw new Error("We could not create your account. Please try again.");
        if (data.user.email_confirmed_at) { toast.success("Account created successfully"); void navigate({ to: "/", replace: true }); }
        else { switchMode("verify"); toast.success("Verification code sent. Check your email inbox and spam folder."); }
      } else if (mode === "verify") {
        const { error } = await supabase.auth.verifyOtp({ email: email.trim(), token: code.trim(), type: "email" });
        if (error) throw error; toast.success("Email verified successfully"); void navigate({ to: "/", replace: true });
      } else {
        const { error } = await supabase.auth.resetPasswordForEmail(email.trim(), { redirectTo: `${window.location.origin}/auth?reset=1` });
        if (error) throw error; toast.success("Password reset instructions sent to your email"); switchMode("login");
      }
  ...
  async function resendCode() {
    ...
    try { const { error } = await supabase.auth.resend({ type: "signup", email: email.trim() }); if (error) throw error; toast.success("A new verification email was requested. Check your inbox and spam folder."); }
