...
function Auth() {
  ...
  // Login is deliberately OTP-first. This avoids the old password-session -> signOut ->
  // second OTP-session race that was making existing users appear to do nothing.
  async function sendLoginCode(target: string) {
    ...
    const { error } = await withTimeout(supabase.auth.signInWithOtp({
      email: clean,
      options: { shouldCreateUser: false, shouldSendEmail: true },
    }));
    if (error) throw error;
    beginOtp("login", clean);
    setEmail(clean); setCode(""); setMode("verify");
  }
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (mode === "login") {
        // Do not create, destroy, or switch sessions here. Existing and old users use
        // Supabase's passwordless OTP sign-in directly, with shouldCreateUser=false.
        // The dashboard is unreachable until verifyOtp returns a real session.
      ...
      if (mode === "verify") {
        ...
        // Both current signup and login email OTPs are verified with type=email in the
        // current Supabase API. The old signup type is deprecated for email OTPs.
