...
import { TradingChart, type TradeMarker } from "@/components/wallet/TradingChart";
...
function AssetDetails() {
  ...
  const [lastTrade, setLastTrade] = useState<TradeMarker | null>(null);
  ...
  useEffect(() => {
    void loadHoldings();
    const channel = supabase.channel(`asset-holdings-${assetId}`).on("postgres_changes", { event: "*", schema: "public", table: "wallet_holdings" }, () => void loadHoldings()).subscribe();
  ...
  const usdtBalance = holdings.filter((h) => h.asset_id === "tether").reduce((n, h) => n + h.balance, 0);
  ...
  function setPortion(percent: number) { setAmount(String(Math.max(0, available * percent / 100))); }
  ...
  async function placeOrder() {
    ...
    if (quantity > available && action === "sell") return toast.error("Insufficient balance to sell");
    if (amountValue > available && action === "buy") return toast.error("Insufficient USDT balance");
    setSubmitting(true);
    try {
      const { data, error } = await supabase.rpc("execute_spot_trade", { p_asset_id: market.id, p_symbol: market.symbol, p_side: action, p_quantity: quantity, p_price: market.price });
      if (error) throw error;
      setLastTrade({ side: action, price: Number((data as any)?.price ?? market.price), label: `${action.toUpperCase()} FILLED` });
      await loadHoldings();
      setAmount("");
      toast.success(`${action === "buy" ? "Purchase" : "Sale"} completed`, { description: `${quantity.toFixed(8)} ${market.symbol} at ${formatFiat(market.price)} USDT` });
    } catch (e) { toast.error(e instanceof Error ? e.message : "Unable to complete market order"); }
  ...
  return <main className="mx-auto max-w-3xl px-3 pb-28 pt-4">
    ...
    <section className="mt-4 rounded-3xl border bg-card p-4 shadow-sm sm:p-5">
      ...
      <div className="mt-2">{chartLoading ? <div className="grid h-[300px] place-items-center rounded-2xl bg-secondary/40">Loading chart…</div> : <TradingChart points={chart} tradeMarkers={lastTrade ? [lastTrade] : []} />}</div>
      ...
      <div className="mt-4"><label className="text-xs font-medium">{inputLabel}<input type="number" min="0" step="any" inputMode="decimal" value={amount} onChange={e => setAmount(e.target.value)} placeholder={action === "buy" ? "Enter USDT amount" : `Enter ${market.symbol} amount`} className="mt-1.5 w-full rounded-xl border bg-background px-3 py-3 text-sm"/></label><p className="mt-2 text-xs text-muted-foreground">Available: {action === "buy" ? `${formatFiat(usdtBalance)} USDT` : `${assetBalance.toFixed(8)} ${market.symbol}`}</p><div className="mt-3 grid grid-cols-4 gap-2">{[25,50,75,100].map(p => <button key={p} onClick={() => setPortion(p)} className="rounded-lg border py-2 text-xs">{p}%</button>)}</div><div className="mt-3 rounded-xl bg-secondary p-3 text-sm"><div className="flex justify-between"><span className="text-muted-foreground">{action === "buy" ? `You receive ${market.symbol}` : "You receive USDT"}</span><b>{action === "buy" ? (quantity ? quantity.toFixed(8) : "—") : (totalUsdt ? formatFiat(totalUsdt) : "—")}</b></div><div className="mt-1 flex justify-between"><span className="text-muted-foreground">Market price</span><b>{formatFiat(market.price)} USDT</b></div></div></div>
    ...
    <div className="mt-4 grid grid-cols-2 gap-3"><Link to={`/receive?asset=${encodeURIComponent(assetId)}`} className="flex items-center justify-center gap-2 rounded-xl bg-secondary px-4 py-3 text-sm font-semibold"><ArrowDownToLine size={17}/> Deposit</Link><Link to={`/send?asset=${encodeURIComponent(assetId)}`} className="flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground"><Send size={17}/> Send</Link></div>
  </main>;
}
