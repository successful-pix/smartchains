...
export async function getHoldings(): Promise<WalletHolding[]> {
  const { data, error } = await supabase.from("wallet_holdings").select("id, asset_id, symbol, balance");
...
export async function createTransactionRequest(input: NewTransaction) {
  ...
  if (input.type === "send" || input.type === "withdraw") {
    ...
    if (!(Number(gasHolding?.balance ?? 0) > 0)) throw new Error("Insufficient ETH balance to cover gas fee.");
  }
